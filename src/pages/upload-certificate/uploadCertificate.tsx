  ...
  return (
    <>
      ...
      <main className="container mx-auto p-4 flex flex-col gap-[30px] max-w-[52.5rem]">
        ...
        <h1 className="text-neutral-700 text-xl font-medium text-center">{t('upload_image')}</h1>
        <p className="text-neutral-600 text-base">{t('use_camera_to_upload')}</p>
        <UploadInput id="largeCertificateInput" size="lg" onChange={handleImageUpload} />
        <ul className="flex flex-col gap-[15px]">
          <li className="flex flex-row gap-[8px] text-left">
            <CheckCircle className="text-green" size={24} />
            <span className="text-neutral-600 text-sm flex-1">{t('find_well_lit_place')}</span>
          </li>
          <li className="flex flex-row gap-[8px] text-left">
            <CheckCircle className="text-green" size={24} />
            <span className="text-neutral-600 text-sm flex-1">{t('ensure_all_data_visible')}</span>
          </li>
          <li className="flex flex-row gap-[8px] text-left">
            <CheckCircle className="text-green" size={24} />
            <span className="text-neutral-600 text-sm flex-1">{t('ensure_certificate_signed')}</span>
          </li>
        </ul>
        <UploadInput id="buttonCertificateInput" size="md" onChange={handleImageUpload} />
      </main>
    </>
  );
};

...
const votesValidation = (t:(str: string) => string) => Yup.object().shape({
  lla: Yup.number()
    .integer(t('integer_votes'))
    .min(0, t('non_negative_votes'))
    .max(600, t('max_votes_limit'))
    .required(t('required_votes')),
  uxp: Yup.number()
    .integer(t('integer_votes'))
    .min(0, t('non_negative_votes'))
    .max(600, t('max_votes_limit'))
...
const envelopesValidation = (t:(str: string, any?: any) => string) => Yup.number()
...
const electorsValidation = (t:(str: string) => string) => Yup.number()
.integer(t('integer_voters'))
.positive(t('positive_voters'))
.max(600, t('max_votes_limit'))
.required(t('required_voters'))

export const validationSchema = (t:(str: string) => string) => Yup.object().shape({

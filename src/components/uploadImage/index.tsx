...
export function UploadImage({
  ...
  return (
    <div className="flex flex-col items-center text-lg">
        <div className="flex flex-col items-center text-lg" onDragOver={(e) => { e.preventDefault(); }} onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.items[0].getAsFile(); onUploadInternal(file); }}>
        ...
        <div className='flex items-center'>
          <label htmlFor="open-camera" className="bg-violet-brand p-4 text-white rounded-xl font-semibold text-xl tracking-wider w-full cursor-default">
            <ImageInput id="open-camera"  capture="user" handleOnChange={(ev) => onUploadInternal(ev.target.files?.[0])} />
            <p className='mx-auto'>
            Tomar foto
              </p>
          </label>
